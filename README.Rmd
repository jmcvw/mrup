...
The default search location is `~/R`, but this can be changed by clicking the `Change` link[^1].
...
The chosen project can then be opened in either the current session or a new one.
...
The search directory (i.e the circled input) is the same directory chosen on the __Open project__ tab.
...
[^1]: Thanks to the [`shinyFiles`](https://github.com/thomasp85/shinyFiles) packages creators, Thomas Lin Pedersen et al.
